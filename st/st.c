...
static char *base64enc(const char *);
...
base64enc(const char *src)
{
	...
	if (in_len > (SIZE_MAX / 4) - 1)
		return NULL;
	result = dst = xmalloc(((in_len + 2) / 3) * 4 + 1);

	while (in_len >= 3) {
		*dst++ = b64[(src[0] >> 2) & 0x3F];
		*dst++ = b64[((src[0] & 0x03) << 4) | ((src[1] >> 4) & 0x0F)];
		*dst++ = b64[((src[1] & 0x0F) << 2) | ((src[2] >> 6) & 0x03)];
		*dst++ = b64[src[2] & 0x3F];
		src += 3;
		in_len -= 3;
	}
		...
		case 52: /* manipulate selection data */
			if (narg > 2 && allowwindowops) {
				if (!strcmp(strescseq.args[2], "?")) {
					/* reply with the clipboard, base64 encoded */
					enc = base64enc(xgetsel());
					if (enc) {
						ttywrite("\033]52;", 5, 1);
						ttywrite(strescseq.args[1],
						         strlen(strescseq.args[1]), 1);
						ttywrite(";", 1, 1);
						ttywrite(enc, strlen(enc), 1);
						ttywrite("\007", 1, 1);
						free(enc);
					}
					return;
