...
(function() {
    ...
    const cssSelectorArr = [
        ...
        `ytd-rich-item-renderer:has(.ytd-display-ad-renderer)`,//首页广告容器完整移除
        `ytd-rich-section-renderer:has(ytd-ad-slot-renderer)`,//首页广告区块完整移除
        ...
        // Additional selectors for empty containers
        `ytd-video-renderer:has(ytd-display-ad-renderer)`,//视频容器中的广告
        `ytd-reel-shelf-renderer:has(ytd-ad-slot-renderer)`,//shorts广告容器
        `.ytd-compact-promoted-video-renderer`,//相关视频中的推广
        `ytd-watch-next-secondary-results-renderer > ytd-compact-promoted-video-renderer`,//右侧推广视频
        `ytd-horizontal-card-list-renderer:has(ytd-ad-slot-renderer)`,//横向广告列表
        `ytd-rich-shelf-renderer:has(ytd-ad-slot-renderer)`,//横架式广告
    ];
