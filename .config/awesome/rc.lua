...
-- {{{ Menu
local myawesomemenu = awful.menu({
	items = {
		{
			"hotkeys",
			function()
...
local load_widget = wibox.widget.textbox()
local io_widget = wibox.widget.textbox()

-- Update system stats every 3 seconds
gears.timer.start_new(3, function()
	-- Get CPU usage (from /proc/stat)
	local cpu_usage = "N/A"
	local f = io.popen("top -bn1 | grep 'Cpu(s)' | sed 's/.*, *\\([0-9.]*\\)%* id.*/\\1/' | awk '{print 100 - $1}'")
	if f then
		cpu_usage = f:read("*a"):gsub("%s+", ""):gsub("\n", "") .. "%"
		f:close()
	end
	cpu_widget:set_text("CPU:" .. cpu_usage .. " ")

	-- Get memory usage
	local mem_usage = "N/A"
	f = io.popen("free | grep Mem | awk '{printf \"%.1f%%\", $3/$2*100}'")
	if f then
		mem_usage = f:read("*a"):gsub("%s+", ""):gsub("\n", "")
		f:close()
	end
	mem_widget:set_text("MEM:" .. mem_usage .. " ")
	...
	load_widget:set_text("LOAD:" .. load_avg .. " ")

	-- Get disk I/O (from iostat)
	local io_usage = "N/A"
	f = io.popen("iostat -x 1 1 | awk 'NR==4 {printf \"R:%.1f W:%.1f\", $4, $5}' 2>/dev/null")
	if f then
		local result = f:read("*a"):gsub("%s+", ""):gsub("\n", "")
		if result ~= "" then
			io_usage = result
		end
		f:close()
	end
	io_widget:set_text("IO:" .. io_usage .. " ")
...
local globalkeys = gears.table.join(
	...
	awful.key({ modkey }, "o", function()
		awful.spawn("cat ~/.pvt/pw | xclip -selection c")
	end),
	...
	awful.key({ modkey }, "v", function()
		awful.spawn("~/.scripts/screen_record.sh start DP-0")
	end),
	awful.key({ modkey, "Shift" }, "v", function()
		awful.spawn("~/.scripts/screen_record.sh stop")
	end),

	-- Wallpaper
	awful.key({ modkey, "Shift" }, "t", function()
		awful.spawn("~/.scripts/randwall_new.sh")
	end),
	awful.key({ modkey, "Shift" }, "n", function()
		awful.spawn("~/.scripts/randwall.sh ~/.pvt/NSFW/")
	end),
...
-- {{{ Client keys
local clientkeys = gears.table.join(awful.key({ modkey }, "q", function(c)
	c:kill()
end))
-- }}}
